...
def test(net, testloader):
    ...
    r2_accuracy = np.mean(r2_scores)
    #mse_accuracy = mse_scores / total
    mse_accuracy = np.mean(mse_scores)
    #mae_accuracy = mae_scores / total
    mae_accuracy = np.mean(mae_scores)
...
def evaluate(
    ...
    losses.append(loss)
    mse_losses.append(mse_accuracy)
...
dataset_name = dataloaders.Dataloader_life_expectancy.get_dataset_name()
# Run dataloader file to make data available.
clean_data_df = dataloaders.Dataloader_life_expectancy.get_clean_dataset()
X_feature, y_feature = dataloaders.Dataloader_life_expectancy.get_data_features()
...
for train_index, test_index in kf.split(clean_data_df):
    ...
    dataloaders.Dataloader_life_expectancy.dl_dataloading(df_train, df_test, X_feature, y_feature)
    ...
    #dataloaders.Dataloader_life_expectancy.dl_dataloading(df_train, df_val, X_feature, y_feature)
    ...
    # Add results
    results_dict = {'dataset': dataset_name, 'R2': accuracies[-1], 'MSE': mse_losses[-1], 'MAE': mae_losses[-1]}
